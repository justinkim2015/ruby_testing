...
# rspec spec/15a_binary_game_spec.rb
...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        input = 3
        value = game.verify_input(min, max, input)
  ...
  describe '#display_turn_order' do
    ...
    let(:binary) { instance_double(BinarySearch) }
    # Create a new subject and an instance_double for BinarySearch.

    before do
      # You'll need to create a few method stubs.
      allow(binary).to receive(:make_guess)
      allow(binary).to receive(:update_range)
      allow(game_turn).to receive(:display_guess)
      allow(binary).to receive(:game_over?)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(binary) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary).to receive(:make_guess).once
      game_turn.display_turn_order(binary)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary).to receive(:update_range).once
      game_turn.display_turn_order(binary)
    end
