...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        solution = game_guessing.make_guess(game_guessing.min, game_guessing.max)
    ...
    context 'when min is 5 and max is 9' do
      let (:game59) { described_class.new(5, 9 ,number_guessing) }

      it 'returns 7' do
        solution = game59.make_guess(game59.min, game59.max)
        expect(solution).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      let (:game89) { described_class.new(8, 9 ,number_guessing) }

      it 'returns 8' do
        solution = game89.make_guess(game89.min, game89.max)
        expect(solution).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      let (:game03) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        solution = game03.make_guess(game03.min, game03.max)
        expect(solution).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      let (:game3) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        solution = game3.make_guess(game3.min, game3.max)
        expect(solution).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let (:rand_num) { double('rand_num', value: 8) }
    subject(:game) { described_class.new(0, 9, rand_num, 8)}
    ...
    context 'when guess and random_number are not equal' do
      let (:untrue_game) { described_class.new(0, 9, rand_num, 4) }
      it 'is not game over' do
        expect(untrue_game.game_over?).not_to eq true
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      it 'updates min to 5' do
        solution = low_guess_game.update_range.min
        expect(solution).to eq(5)
      end

      it 'does not update max' do
        solution = low_guess_game.update_range.max
