...
# rspec spec/13_input_output_spec.rb
...
describe NumberGame do
  ...
  describe '#initialize' do
    context 'when solution is initialized' do
      ...
      matcher :be_between_zero_and_nine do
        match  { |game| game.solution.between?(0, 9) }
      end

      # remove the 'x' before running this test
      it 'is a number between 0 and 9' do
        expect(game).to be_between_zero_and_nine
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      # remove the 'x' before running this test
      subject(:wrong_guess) { described_class.new(5, '3')}
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_number = '390'
        valid = '3'
        allow(game_loop).to receive(:player_input).and_return(invalid_letter, invalid_number, valid)
      end
  ...
  describe '#final_message' do
    ...
    # @guess, and @count
    subject(:game_two_three) { described_class.new(5, '5', 3)}

    context 'when count is 2-3' do
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_two_three.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    # Write a test for the following context.
    subject(:game_four) { described_class.new(5, '5', 6)}
